...
public class BillSummaryView extends VerticalLayout {
    ...
    Button promoCode = new Button("Avail Promo Discount");
    H4 newTotalCost = new H4("");
    ...
    public BillSummaryView() {
        ...
        promoCode.addClickListener(e -> {
            if (customer.getCount() >= 1) {
                totalCost = totalCost - (totalCost) / 20;
                newTotalCost.setText("Total Cost after discount of 5%: " + totalCost);
            } else {
                newTotalCost.setText("Promo code cannot be applied.");
            }
            getPaymentDetails();
            ...
            Command onPaymentGatewayRequestComplete = () -> {
                ...
                Notification
                        .show("Purchase complete! \nWe have charged the total (Rs." + totalCost + ") from your credit card ending in "
                                + cardEnding, 3000, Position.MIDDLE);
